...
def main():
    ...
    try:
        ...
        return
    fix_snmp_datatypes()
    snmpwalk = parse_snmpwalkfile(filename)
    if command == 'dump':
        pprint(snmpwalk)
    elif command == 'serve':
        simulate(
...
File should be generated by snmpwalk with numeric OIDs
Like this:
snmpwalk -v2c -On -c $COMMUNITY_STRING $HOST $START_OID > $FILENAME
'''
...
def fix_snmp_datatypes():
    ...
    fixed_v1Mapping = [(
        rfc1902.Counter64,
        datatypes.SimpleConverter(v2c.Counter64)
    )]
